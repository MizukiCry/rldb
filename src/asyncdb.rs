...
use crate::{Database, Options, Result, Status, StatusCode, WriteBatch};
...
impl AsyncDB {
    ...
    pub fn new<P: AsRef<Path>>(name: P, opts: Options) -> Result<AsyncDB> {
        let db = Database::open(name, opts)?;
    ...
    fn run_server(mut db: Database, mut recv: mpsc::Receiver<Message>) {
